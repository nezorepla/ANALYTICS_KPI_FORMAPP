...
namespace ExtensionMethods
{
    public static class Extension
    {
        ...
        public static string ToFormat(this double str)
        {
            string n = str.ToString("#,##0");
...
namespace ANALYTICS_KPI_FORMAPP
{
    public partial class Form1 : Form
    {
        ...
        public static string NormTable(DataTable dt, string file, string parent, string trend, string baslik, string aciklama)
        {
            ...
            string rv = "<h3>(<u>" + parent + "</u>) " + baslik + "</h3><font color=\"GRAY\">" + aciklama + "</font><table><tr><td><table class=\"NormTable\"><tr><th></th><th>Değer (" + MaxTarih.ToString() + ")</th><th>Ay Ortalaması</th><th>3 Aylık Ortalama</th><th>3-6 Aylık Ortalama</th><th>3 Aylık St. Sapma</th><th>2014 tibari İle Ortalama</th><th>2014 itibari ile St. Sapma</th><th>Trend</th><th>Aylık Trend</th><th>Max</th><th>Min</th></tr>";
            ...
            for (int i = 1; i < dt.Columns.Count; i++)
            {
                ...
                double StandartSapma3Aylik = Math.Round(StDev(s), 2);

                s = Seri(dt, name, MaxTarih);
                double Deger12Aylik = Math.Round(Mean(s), 2);
                double StandartSapma12Aylik = Math.Round(StDev(s), 2);


                double AltSinir=0;
                if (Deger12Aylik - StandartSapma12Aylik > 0)
                    AltSinir = (Deger12Aylik - StandartSapma12Aylik);

                rv += "<tr><td>" + name
                    + "</td><td class=\"N\">" + Deger.ToFormat()
                    + "</td><td class=\"N\">" + DegerAylik.ToFormat()
                    + "</td><td class=\"N\">" + Deger3Aylik.ToFormat()
                    + "</td><td class=\"N\">" + Deger6Aylik.ToFormat()
                    + "</td><td class=\"N\">" + StandartSapma3Aylik.ToFormat()
                    + "</td><td class=\"N\">" + Deger12Aylik.ToFormat()
                    + "</td><td class=\"N\">" + StandartSapma12Aylik.ToFormat()
                    + "</td><td><img width=\"12\" height=\"12\" src=\"" + savePath + "trend" + Trend(Deger, Deger3Aylik, StandartSapma3Aylik, trend) + ".png\">"
                    + "</td><td><img width=\"12\" height=\"12\" src=\"" + savePath + "trend" + Trend(DegerAylik, Deger3Aylik, StandartSapma3Aylik, trend) + ".png\">"
                    + "</td><td class=\"N\">" + (Deger12Aylik+StandartSapma12Aylik).ToFormat()
                    + "</td><td class=\"N\">" + AltSinir.ToFormat()
                    + "</td></tr>";
        ...
        public static double[] Seri(DataTable dt, string Column, int Tarih, int DahilAy)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                int dint = int.Parse(dt.Rows[i][0].ToString().Substring(0, 6));
                if ((dint != newDt) && (dint >= iDt))
                {
